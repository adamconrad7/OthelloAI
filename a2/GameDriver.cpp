...
//generate all succesors of a given state
//repeat for all possible moves:
	//try a move
	//if legal, add to frontier
//return frontier

// void GameDriver::step( Player* p){
// 	for(int i = 0; i<4; i++){
// 		for(int j=0; j<4; j++){
// 			if(	board->is_legal_move(i, j, p->get_symbol()) ){
// 				std::cout<<"C: "<<i<<" R: "<<j<<std::endl;
//
// 			}
//
// 		}
...
void GameDriver::process_move(Player* curr_player, Player* opponent) {
	...
	while (invalid_move) {
	//	std::cout<<"C: "<<col<<" R: "<<row<<std::endl;
	//	curr_player->step(board, curr_player->get_symbol());
		curr_player->get_move(board, col, row, curr_player->get_symbol());
		if (!board->is_legal_move(col, row, curr_player->get_symbol())) {
			...
			//Prints every move current player can make
//step(curr_player);
